...
namespace Audio{
    ...
    esp_err_t pipeline_show_song(){
        ...
        if(sdcard_list_prev(sdcard_list_handle, 1, &previous) == ESP_FAIL){
        ...
        if(sdcard_list_next(sdcard_list_handle, 1, &next) == ESP_FAIL){
        ...
        Wifi::send_websocket_message(Wifi::websocket_req_handle, json_str);
